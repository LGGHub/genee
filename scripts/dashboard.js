...
async function cytobandLookup(pairs) {
  ...
  if (pairs) cytobands = getCytobandPairs(cytobands).map(p => p.join(' '));

  fillTextArea('terms', cytobands);
  renderGoogleScholarTable();
}
...
function renderGoogleScholarTable() {
  let terms = getTextareaContents('terms')
    ...
    , queries = buildGoogleScholarQueries(qterms)
  ...
  if (!urls || !urls.length) {
    $('#google-scholar-searches').addClass('d-none');
    return;
  }

  $('#google-scholar-searches').removeClass('d-none').find('tbody').html(urls.map(u => {
    return `
      <tr>
        <td name="query">${u.query}</td>
        <td class="text-end">
          <a class="btn btn-success" href="${u.url}" target="_blank">Google Scholar Search</a>
        </td>
      </tr>
    `;
  }).join('\n'));
...
$('#cytoband-lookup a').on('click', e => {
  ...
  cytobandLookup(val === 'cytoband-pairs' ? true : false);
});
